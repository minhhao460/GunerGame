...
public class Enemy : LivingOB
{
    ...
    void UpdateEffect()
    {
        ...
        if (freezing)
        {
            tilemove_anim = tilemove_anim - BulletManager.Tile_DeMove;
            tilemove = tilemove - BulletManager.Tile_DeMove;
            DeCreaseBang();
        } else
        if (stuning)
        {
    ...
    void NhanLua()
    {
        ...
        if (time_bang > 0)
        {
            KhacChe("Phá Băng", BulletManager.color_Lua + BulletManager.color_Bang);
            ResetBang();
            return;
        }
    ...
    void NhanBang()
    {
        ...
        if (damage_Lua > 0)
        {
            KhacChe("Phá Hỏa", BulletManager.color_Bang + BulletManager.color_Lua);
            ResetLua();
            return;
        }
